...
class UserControllerTest(unittest.TestCase):
    ...
    def test_login_user_should_return_200_and_authorization_token(self):
        ...
        self.assertEquals(response.json().get('status_code'), 202)
        self.assertEquals(response.json().get('detail'), 'Accepted')
    ...
    def test_login_user_should_return_401_if_password_is_wrong(self):
        ...
        self.assertEquals(response.json().get('status_code'), 401)
        self.assertEquals(response.json().get('detail'), 'Unauthorized')
    ...
    def test_login_user_should_return_404_if_user_is_not_found(self):
        ...
        self.assertEquals(response.json().get('status_code'), 404)
        self.assertEquals(response.json().get('detail'), 'Not found')
